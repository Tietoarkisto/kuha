...
def update(provider, since=None, purge=False):
    ...
    HarvestError:
        ...
        continued.
    """
    update_formats(provider, purge)
    update_items(provider, purge)
    update_records(provider, since)


def update_formats(provider, purge=False):
...
def update_items(provider, purge=False):
    log = logging.getLogger(__name__)
    log.info('Looking for added and removed items...')
...
def update_sets(provider, identifier):
    log = logging.getLogger(__name__)
...
def update_records(provider, since=None):
    log = logging.getLogger(__name__)
    ...
    prefixes = [f.prefix for f in models.Format.list(ignore_deleted=True)]
    identifiers = [i.identifier
                   for i in models.Item.list(ignore_deleted=True)]
    ...
    for identifier in identifiers:
        try:
            ...
            update_sets(provider, identifier)
        except Exception as e:
            log.exception(
